...
func (c *Client) doInternal(ctx context.Context, method, path string, params map[string]string, body any, headerOverrides map[string]string, readOnlyIntent bool, mutationIntent bool) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if req.Header.Get("User-Agent") == "" {
			req.Header.Set("User-Agent", "exa-pp-cli/2.0.0")
		}
		...
		if resp.StatusCode < 400 {
			...
			RecordCostJournal(method, path, sanitized)
...
func authPlaceholderCredentialError(cfg *config.Config) error {
	return authPlaceholderCredentialErrorWithSetup(cfg, "export EXA_API_KEY=<your-token> or exa-pp-cli auth set-token <token>")
}
...
func (c *Client) maskCredentialText(text string, extraCredentials ...string) string {
	...
	if c != nil && c.Config != nil {
		...
		addCredential(c.Config.ExaApiKey)
	}
...
// CostJournalPath returns the JSONL cost journal path under the CLI data dir.
// Every live response carrying costDollars is appended here; the `spend`
// command aggregates it. Best-effort: failures never fail the API call.
func CostJournalPath() string {
	dir, err := cliutil.DataDir()
	if err != nil {
		if home, homeErr := os.UserHomeDir(); homeErr == nil {
			return filepath.Join(home, ".local", "share", "exa-pp-cli", "cost.jsonl")
		}
		return "cost.jsonl"
	}
	return filepath.Join(dir, "cost.jsonl")
}

// RecordCostJournal appends one line per billed Exa response. Skipped in
// verify/dogfood envs so test matrices do not pollute real spend data.
func RecordCostJournal(method, path string, body []byte) {
	if cliutil.IsVerifyEnv() || cliutil.IsDogfoodEnv() {
		return
	}
	var envelope struct {
	...
	if err := json.Unmarshal(body, &envelope); err != nil || envelope.CostDollar.Total == 0 {
		return
	}
	entry, err := json.Marshal(map[string]any{
		...
		"cost":      envelope.CostDollar.Total,
	})
	if err != nil {
		return
	}
	journalPath := CostJournalPath()
