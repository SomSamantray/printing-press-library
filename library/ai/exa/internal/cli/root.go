...
type rootFlags struct {
	...
	// invocation. Mirrors the EXA_NO_LEARN env var.
	noLearn bool
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "exa-pp-cli",
		Short: `Exa CLI — Every Exa search API feature, plus local history, spend tracking, and diffing no other Exa tool has.`,
		Long: `Exa CLI — Every Exa search API feature, plus local history, spend tracking, and diffing no other Exa tool has.
...
Health check: run 'exa-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("exa-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'exa-pp-cli profile list')")
	...
	rootCmd.AddCommand(newWebsetsCmd(flags))
	...
	rootCmd.AddCommand(newAnswerPromotedCmd(flags))
	rootCmd.AddCommand(newContentsPromotedCmd(flags))
	rootCmd.AddCommand(newFindSimilarPromotedCmd(flags))
	rootCmd.AddCommand(newTeamsPromotedCmd(flags))
	rootCmd.AddCommand(newWebsearchPromotedCmd(flags))
	...
	addNovelCommandIfAbsent(rootCmd, newNovelEntityCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelMonitorCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelSpendCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelWebsetCmd(flags))
