...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("EXA_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("EXA_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "exa-pp-cli/feedback")
...
When ` + "`EXA_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`EXA_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  exa-pp-cli feedback list
  exa-pp-cli feedback list --limit 5
  exa-pp-cli feedback list --json`,
