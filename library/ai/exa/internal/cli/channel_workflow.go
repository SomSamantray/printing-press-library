...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  exa-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  exa-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"agent", "events", "imports", "monitors", "webhooks", "websets"}
			archiveMaxPages := 100
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  exa-pp-cli workflow status

  # Show status as JSON
  exa-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("exa-pp-cli")
			}
