...
Resource scoping:
  ...
  exa-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  exa-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  exa-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  exa-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  exa-pp-cli sync --latest-only`,
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "answer":
		return db.UpsertAnswer(data)
	case "contents":
		return db.UpsertContents(data)
	case "find-similar":
		return db.UpsertFindSimilar(data)
	case "imports":
		return db.UpsertImports(data)
	case "monitors":
		return db.UpsertMonitors(data)
	case "runs":
	...
	case "attempts":
		return db.UpsertAttempts(data)
	case "websets":
		return db.UpsertWebsets(data)
	case "cancel":
		return db.UpsertCancel(data)
	case "enrichments":
		return db.UpsertEnrichments(data)
	case "items":
		return db.UpsertItems(data)
	case "searches":
		return db.UpsertSearches(data)
	default:
...
func dependentResourceDefs() []dependentResourceDef {
	return []dependentResourceDef{
		{Name: "attempts", ParentTable: "webhooks", ParentIDParam: "id", PathTemplate: "/v0/webhooks/{id}/attempts", KeyField: "", ReconcileMode: "per_parent", GenericScopeJSONPath: "$.webhook", PathParams: []dependentPathParamDef{
			{Param: "id", Field: "id"},
		}},
		{Name: "items", ParentTable: "websets", ParentIDParam: "webset", PathTemplate: "/v0/websets/{webset}/items", KeyField: "", ReconcileMode: "per_parent", GenericScopeJSONPath: "$.webset", PathParams: []dependentPathParamDef{
			{Param: "webset", Field: "id"},
		}},
		{Name: "runs", ParentTable: "monitors", ParentIDParam: "id", PathTemplate: "/monitors/{id}/runs", KeyField: "", ReconcileMode: "per_parent", GenericScopeJSONPath: "$.monitor", PathParams: []dependentPathParamDef{
			{Param: "id", Field: "id"},
		}},
	}
}
...
func responsePathForResource(resource, path string) []string {
	...
	case "agent\x00/agent/runs/{id}/events":
		return []string{"data"}
	case "agent\x00/agent/runs":
		return []string{"data"}
	case "answer\x00/answer":
		return []string{"citations"}
	case "events\x00/v0/events":
		return []string{"data"}
	case "find-similar\x00/findSimilar":
		return []string{"results"}
	case "imports\x00/v0/imports":
		return []string{"data"}
	case "monitors\x00/monitors/batch":
		return []string{"ids"}
	case "monitors\x00/monitors":
		return []string{"data"}
	case "monitors\x00/v0/monitors":
		return []string{"data"}
	case "webhooks\x00/v0/webhooks":
		return []string{"events"}
	case "webhooks\x00/v0/webhooks/{id}":
		return []string{"events"}
	case "websets\x00/v0/websets":
		return []string{"data"}
	}
	return nil
}
