...
In local mode: searches locally synced data only.`,
  ...
  exa-pp-cli search "error timeout"

  # Force local search only
  exa-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  exa-pp-cli search "status" --type agent --data-source local
  # JSON output for piping
  exa-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if flags.dataSource != "local" {
				...
				data, _, getErr := c.Post(cmd.Context(), "/search", map[string]any{
					"query":              query,
					"additionalQueries":  "",
					"category":           "",
					"compliance":         "",
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'exa-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "answer":
				results, err = db.SearchAnswer(query, limit)
			case "find-similar":
				results, err = db.SearchFindSimilar(query, limit)
			case "monitors":
				results, err = db.SearchMonitors(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchAnswer(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search answer failed: %w", searchErr)
					}
				...
				{
					partial, searchErr := db.SearchFindSimilar(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search find_similar failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchMonitors(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search monitors failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
