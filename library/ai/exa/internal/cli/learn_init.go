...
func newLearnConfig() *entities.Config {
	...
	if re, err := regexp.Compile(`^https?://[a-zA-Z0-9.-]+\.[a-z]{2,}(/[^ ]*)?$`); err != nil {
		fmt.Fprintf(os.Stderr, "warning: exa-pp-cli: ticker pattern %q failed to compile: %v\n", `^https?://[a-zA-Z0-9.-]+\.[a-z]{2,}(/[^ ]*)?$`, err)
	} else {
...
func initLearn(ctx context.Context, db *sql.DB) error {
	...
	seeds := map[string][]lookups.SeedConfig{
		"category": {
			{Canonical: "company", Values: []string{"companies", "organizations", "businesses"}},
			{Canonical: "people", Values: []string{"person", "individuals", "profiles"}},
			{Canonical: "news", Values: []string{"current events", "journalism", "articles"}},
			{Canonical: "publication", Values: []string{"research papers", "papers", "academic"}},
			{Canonical: "financial report", Values: []string{"sec filings", "earnings", "financials"}},
			{Canonical: "personal site", Values: []string{"blogs", "personal pages"}},
		},
		"search_type": {
			{Canonical: "auto", Values: []string{"default", "balanced"}},
			{Canonical: "fast", Values: []string{"quick"}},
			{Canonical: "instant", Values: []string{"low latency", "realtime", "real-time"}},
			{Canonical: "deep", Values: []string{"deep research", "thorough"}},
			{Canonical: "deep-reasoning", Values: []string{"maximum reasoning"}},
			{Canonical: "deep-lite", Values: []string{"lightweight deep"}},
		},
	}
...
func runLearnInitOnce(ctx context.Context) {
	learnInitOnce.Do(func() {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "warning: exa-pp-cli: learn init: open store: %v\n", err)
			return
		}
		defer s.Close()
		if err := initLearn(ctx, s.DB()); err != nil {
			fmt.Fprintf(os.Stderr, "warning: exa-pp-cli: learn init: %v\n", err)
		}
	})
}
