...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  exa-pp-cli doctor
  exa-pp-cli doctor --json
  exa-pp-cli doctor --fail-on warn
  exa-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if !configured {
					...
					report["auth_hint"] = "Set it with: exa-pp-cli auth set-token <token> or export EXA_API_KEY=\"your-token-here\""
				} else {
					authConfigured = true
			...
			if os.Getenv("EXA_API_KEY") != "" {
				authEnvSet = append(authEnvSet, "EXA_API_KEY")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "EXA_API_KEY")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						authHeaders["User-Agent"] = "exa-pp-cli"
						verifyPath := "/v0/teams/me"
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'exa-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'exa-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		...
		report["hint"] = "No sync recorded; run 'exa-pp-cli sync' to hydrate API-backed resources. Rows written by other paths are not tracked in sync_state."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'exa-pp-cli sync' to refresh."
	}
	return report
}
