...
func RegisterIntents(s *server.MCPServer) {
	s.AddTool(
		mcplib.NewTool("what_changed_since_last_monitor_run",
			mcplib.WithDescription("Compare the two most recent synced runs and see exactly which URLs appeared or disappeared."),
			mcplib.WithString("monitor_id", mcplib.Required(), mcplib.Description("Override the recipe's positional monitor_id value.")),
		),
		handleWhatChangedSinceLastMonitorRun,
	)
}

// handleWhatChangedSinceLastMonitorRun runs the what_changed_since_last_monitor_run recipe intent: Compare the two most recent synced runs and see exactly which URLs appeared or disappeared.
func handleWhatChangedSinceLastMonitorRun(ctx context.Context, req mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
	...
	var missingMonitorId bool
	args, missingMonitorId = appendRecipePositional(args, input["monitor_id"], true)
	if missingMonitorId {
		return mcplib.NewToolResultError("monitor_id is required"), nil
	}
