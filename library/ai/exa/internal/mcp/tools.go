		...
		if err != nil {
			...
			case strings.Contains(msg, "HTTP 400") && cliutil.LooksLikeAuthError(msg):
				return mcpToolError("authentication error: " + cliutil.SanitizeErrorBody(msg) +
					...
					"\n      Set it with: exa-pp-cli auth set-token <token> or export EXA_API_KEY=\"your-token-here\"" +
					"\n      Run 'exa-pp-cli doctor' to check auth status."), nil
...
func openMCPReadOnlyStore(path string) (*store.Store, *mcplib.CallToolResult) {
	...
	if err != nil {
		return nil, mcplib.NewToolResultError(fmt.Sprintf("opening local data store %s: %v. Run exa-pp-cli sync to refresh the store, or use live endpoint MCP tools for unsynced data.", path, err))
	}
	return db, nil
}

func mcpMissingStoreMessage(path string) string {
	return fmt.Sprintf("No local data store found at %s. Run exa-pp-cli sync before using MCP search/sql, or use live endpoint MCP tools for unsynced data.", path)
}
...
func mcpEmptyStoreNextStep() string {
	return "Run exa-pp-cli sync to populate the local SQLite store before using MCP search/sql."
}
...
func handleContext(_ context.Context, _ mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
	...
	ctx := map[string]any{
		"api":         "exa",
		"description": "Every Exa search API feature, plus local history, spend tracking, and diffing no other Exa tool has.",
		"archetype":   "content",
		"tool_count":  58,
		"paths":       paths,
		// tool_surface tells agents which surface a capability lives on.
		"tool_surface": "MCP exposes typed endpoint tools plus a runtime mirror of user-facing CLI commands. Endpoint tools keep typed schemas; command-mirror tools shell out to the companion exa-pp-cli binary.",
		...
		"resources": []map[string]any{
			{
				...
				"endpoints":   []string{"cancel-run", "create-run", "delete-run", "get-run", "list-run-events", "list-runs"},
				"syncable":    true,
				"searchable":  true,
				"writable":    true,
			},
			...
			{
				"name":        "find-similar",
				"description": "Manage find similar",
				"endpoints":   []string{"find_similar"},
				"searchable":  true,
			},
			{
				"name":        "imports",
			...
			},
			{
				...
				"endpoints":   []string{"batch", "create", "create-endpoint", "delete", "delete-id", "get", "get-id", "list", "list-endpoint", "update", "update-id"},
				"syncable":    true,
				"searchable":  true,
				"writable":    true,
			},
			{
				...
				"endpoints":   []string{"get", "list", "monitors-get", "monitors-list"},
			},
			{
			...
			},
			{
				...
				"description": "Manage attempts",
				"endpoints":   []string{"webhooks-list"},
			},
			{
				"name":        "websearch",
			...
			},
			{
				...
				"endpoints":   []string{"create", "delete", "get", "list", "preview", "update"},
				"syncable":    true,
				"searchable":  true,
				"writable":    true,
			},
			{
				...
				"endpoints":   []string{"websets"},
				"writable":    true,
			},
			{
				"name":        "websets.enrichments",
				"description": "Manage enrichments",
				"endpoints":   []string{"websets-cancel", "websets-create", "websets-delete", "websets-get", "websets-update"},
				"writable":    true,
			},
			{
				"name":        "websets.items",
				"description": "Manage items",
				"endpoints":   []string{"websets-delete", "websets-get", "websets-list"},
				"writable":    true,
			},
			{
				"name":        "websets.searches",
				"description": "Manage searches",
				"endpoints":   []string{"websets-cancel", "websets-create", "websets-get"},
				"writable":    true,
			},
		},
		"query_tips": []string{
			...
			"Use createdAfter for incremental fetches (filter by modification time).",
		...
		"command_mirror_capabilities": []map[string]string{
			{"name": "Spend report", "command": "spend", "description": "See cumulative API spend across every Exa call, broken down by day and resource.", "rationale": "Every Exa response carries costDollars.total but no endpoint aggregates spend; only the local store can join cost across calls.", "via": "mcp-command-mirror"},
			{"name": "Monitor run diff", "command": "monitor diff", "description": "Compare two synced monitor runs and see exactly which URLs are new, gone, or unchanged.", "rationale": "Joining two monitor-run result sets is a local-SQLite operation the Exa API cannot express.", "via": "mcp-command-mirror"},
			{"name": "Entity report", "command": "entity report", "description": "Build a first-seen / last-seen / mention-count timeline for any company or person across your synced searches and webset items.", "rationale": "Requires an FTS5 join across webset entity items and search/monitor results that only exists together in the local store.", "via": "mcp-command-mirror"},
			{"name": "Webset new", "command": "webset new", "description": "List items added to a live webset since your last sync, so you only see what changed.", "rationale": "Diffing current synced item state against the previous snapshot is a local join; the API exposes items but not deltas.", "via": "mcp-command-mirror"},
		},
		"playbook": []map[string]string{
			{"topic": "Spend report", "insight": "Every Exa response carries costDollars.total but no endpoint aggregates spend; only the local store can join cost across calls."},
			{"topic": "Monitor run diff", "insight": "Joining two monitor-run result sets is a local-SQLite operation the Exa API cannot express."},
			{"topic": "Entity report", "insight": "Requires an FTS5 join across webset entity items and search/monitor results that only exists together in the local store."},
			{"topic": "Webset new", "insight": "Diffing current synced item state against the previous snapshot is a local join; the API exposes items but not deltas."},
		},
	}
