...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("GROQ_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("GROQ_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "groq-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  groq-pp-cli feedback \"the --limit flag surprised me\"\n  groq-pp-cli feedback list",
		Long: `Feedback is captured locally first in the CLI data directory's feedback.jsonl.
When ` + "`GROQ_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`GROQ_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  groq-pp-cli feedback list
  groq-pp-cli feedback list --limit 5
  groq-pp-cli feedback list --json`,
