...
- Use `--home <dir>` for one invocation, or set `BOLNA_PP_CLI_HOME=<dir>` to relocate all four path kinds under one root.
- Use per-kind env vars only when a specific kind must diverge: `BOLNA_PP_CLI_CONFIG_DIR`, `BOLNA_PP_CLI_DATA_DIR`, `BOLNA_PP_CLI_STATE_DIR`, `BOLNA_PP_CLI_CACHE_DIR`.
...
Fleet precedence: an inherited per-kind env var overrides an explicit `--home` for that kind. Use `BOLNA_PP_CLI_HOME` or per-kind vars as durable fleet levers, and use `--home` only for a single invocation. Relocation is not reversible by unsetting env vars; move files manually before clearing `BOLNA_PP_CLI_HOME`, or `doctor` will not find credentials left under the former root.
...
- `BOLNA_PP_CLI_NO_LEARN=true` in the environment globally disables the pipeline.
