...
func withJournalHome(t *testing.T) string {
	...
	dir := t.TempDir()
	t.Setenv("HOME", dir)
	t.Setenv("BOLNA_PP_CLI_STATE_DIR", "")
	t.Setenv("BOLNA_PP_CLI_HOME", "")
	t.Setenv("XDG_STATE_HOME", "")
	t.Setenv("PRINTING_PRESS_VERIFY", "")
	t.Setenv("PRINTING_PRESS_DOGFOOD", "")
	t.Setenv("BOLNA_PP_CLI_NO_LEARN", "")
	t.Setenv("BOLNA_PP_CLI_LEARN_NO_CAPTURE", "")
	t.Setenv("BOLNA_PP_CLI_LEARN_SESSION", "")
	return dir
}
...
func TestJournal_SessionKeyFromEnv(t *testing.T) {
	withJournalHome(t)
	t.Setenv("BOLNA_PP_CLI_LEARN_SESSION", "session-abc123")
	if err := runCLI(t, "version"); err != nil {
		t.Fatalf("version: %v", err)
	}
	entries := readAllJournalEntries(t)
...
func TestJournal_DisableSwitchesWriteNothing(t *testing.T) {
	cases := []struct {
		...
		{name: "master no-learn env", env: map[string]string{"BOLNA_PP_CLI_NO_LEARN": "1"}},
		{name: "capture-only env", env: map[string]string{"BOLNA_PP_CLI_LEARN_NO_CAPTURE": "1"}},
