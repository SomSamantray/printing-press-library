...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("BOLNA_PP_CLI_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("BOLNA_PP_CLI_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "bolna-pp-cli-pp-cli/feedback")
...
When ` + "`BOLNA_PP_CLI_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`BOLNA_PP_CLI_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  bolna-pp-cli-pp-cli feedback list
  bolna-pp-cli-pp-cli feedback list --limit 5
  bolna-pp-cli-pp-cli feedback list --json`,
