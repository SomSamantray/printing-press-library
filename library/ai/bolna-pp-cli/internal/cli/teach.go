...
// bolna-pp-cli-pp-cli CLI: `teach` (fire-and-forget, silent on success,
...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/ai/bolna-pp-cli/internal/learn/entities"
	"github.com/mvanhorn/printing-press-library/library/ai/bolna-pp-cli/internal/learn/lookups"
	"github.com/mvanhorn/printing-press-library/library/ai/bolna-pp-cli/internal/learn/patterns"
...
const noLearnEnvVar = "BOLNA_PP_CLI_NO_LEARN"
...
func learnDBPath(explicit string) string {
	...
	return defaultDBPath("bolna-pp-cli-pp-cli")
}
...
func newTeachCmd(flags *rootFlags, learnCfg *entities.Config) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Annotations: map[string]string{"pp:typed-exit-codes": "0,2", "pp:interactive": "true", "mcp:local-write": "true"},
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  bolna-pp-cli-pp-cli teach --query "<question>" --resource-type <type> \
...
when learnings exist.`,
		Example: `  bolna-pp-cli-pp-cli recall "<question>" --agent
  bolna-pp-cli-pp-cli recall "<question>" --agent --min-confidence 2`,
...
func newLearningsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "list",
		Short: "List recorded query-to-resource learnings",
		Example: `  bolna-pp-cli-pp-cli learnings list --agent
  bolna-pp-cli-pp-cli learnings list --query "<substring>"
  bolna-pp-cli-pp-cli learnings list --warnings --agent`,
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
...
Requires at least one of --resource, --action, or --all.`,
		Example: `  bolna-pp-cli-pp-cli learnings forget "<question>" --resource <id>
  bolna-pp-cli-pp-cli learnings forget "<question>" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
...
a whole family.`,
		Example: `  bolna-pp-cli-pp-cli teach-pattern \
...
cannot be taught — they are derived from the canonical input.`,
		Example: `  bolna-pp-cli-pp-cli teach-lookup --kind country --canonical "United States" --value USA`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
			}
