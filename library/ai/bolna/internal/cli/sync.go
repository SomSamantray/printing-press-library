...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/ai/bolna/internal/cliutil"
	"github.com/mvanhorn/printing-press-library/library/ai/bolna/internal/learn"
	"github.com/mvanhorn/printing-press-library/library/ai/bolna/internal/learn/lookups"
	"github.com/mvanhorn/printing-press-library/library/ai/bolna/internal/store"
	"context"
	"database/sql"
	"encoding/json"
	"errors"
...
const dogfoodMaxSyncResources = 4
  ...
  bolna-pp-cli-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(resources) == 0 {
				...
				// The generated happy-path example runs `sync` without a resource
				// filter. Keep live dogfood representative but bounded: syncing
				// every Bolna resource fans out into dependent calls and can spend
				// the runner's entire per-command budget on rate-limit waits. An
				// explicit --resources selection remains authoritative.
				if cliutil.IsDogfoodEnv() && len(resources) > dogfoodMaxSyncResources {
					originalResourceCount := len(resources)
					resources = resources[:dogfoodMaxSyncResources]
					if humanFriendly {
						fmt.Fprintf(os.Stderr, "sync: dogfood capped default resource sample to %d of %d resources\n", len(resources), originalResourceCount)
					} else {
						fmt.Fprintf(syncEventWriter, `{"event":"sync_warning","reason":"dogfood_resource_cap_hit","message":"dogfood capped the default sync sample to %d of %d resources; pass --resources to choose an explicit live sample"}`+"\n", len(resources), originalResourceCount)
					}
				}
			}
...
func syncResource(ctx context.Context, c interface {
	...
	for {
		...
		// The dashboard endpoint is tenant-scoped in the live platform. Keep
		// sync usable for the main account by forwarding the same context the
		// platform UI sends when it is available in the environment.
		if resource == "bolna-analytics" {
			if v := os.Getenv("BOLNA_PP_CLI_SUB_ACCOUNT_ID"); v != "" {
				params["sub_account_id"] = v
			}
			if v := os.Getenv("BOLNA_PP_CLI_REQUESTER_EMAIL"); v != "" {
				params["requester_email"] = v
			}
		}
...
func determinePaginationDefaults(resource string) paginationDefaults {
	...
	case "agent":
		return paginationDefaults{
			cursorParam: "page_number",
			cursorType:  "page",
			limitParam:  "page_size",
			limit:       20,
		}
	case "agent/agent_executions":
		return paginationDefaults{
			cursorParam: "page_number",
			cursorType:  "page",
			limitParam:  "page_size",
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "agent_dispositions":
		return db.UpsertAgentDispositions(data)
	case "execution":
		return db.UpsertExecution(data)
	case "agent_executions":
		return db.UpsertAgentExecutions(data)
	case "prompt":
		return db.UpsertPrompt(data)
	case "agent_stop":
		return db.UpsertAgentStop(data)
	case "all":
		return db.UpsertAll(data)
	case "batches_executions":
		return db.UpsertBatchesExecutions(data)
	case "schedule":
		return db.UpsertSchedule(data)
	case "batches_stop":
		return db.UpsertBatchesStop(data)
	case "call_stop":
		return db.UpsertCallStop(data)
	case "log":
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{ // #nosec G101 -- endpoint paths, not credentials.
		...
		"assistants":                  "/assistants/get_gpt_assistants",
		"bolna-analytics":             "/analytics/dashboards",
		"compliance":                  "/compliance/list",
		"dispositions":                "/dispositions/",
		"executions":                  "/batches/{batch_id}/executions",
		"invoices":                    "/invoices",
		"keys":                        "/keys",
		"knowledgebase":               "/knowledgebase/all",
		"phone-numbers":               "/phone-numbers/all",
		"prompt-library":              "/prompt-library/modules",
		"providers":                   "/providers",
		"reports":                     "/reports/jobs",
		"sip-trunks":                  "/sip-trunks/trunks",
		"sub-accounts":                "/sub-accounts/all",
		"sub-accounts-all-usage":      "/sub-accounts/all/usage",
		"user":                        "/user/model/all",
		"user-webhook-allowed-states": "/user/webhook/allowed-states",
		"violations":                  "/violations",
		"voice-config":                "/api/v1/voice-config/languages",
		"voice-config-tts-voices":     "/api/v1/voice-config/tts/voices",
	}
...
func dependentResourceDefs() []dependentResourceDef {
	return []dependentResourceDef{
		{Name: "agent_executions", ParentTable: "agent", ParentIDParam: "agent_id", PathTemplate: "/v2/agent/{agent_id}/executions", KeyField: "", ReconcileMode: "none", GenericScopeJSONPath: "", PathParams: []dependentPathParamDef{
			{Param: "agent_id", Field: "id"},
		}},
		{Name: "all", ParentTable: "agent", ParentIDParam: "agent_id", PathTemplate: "/batches/{agent_id}/all", KeyField: "", ReconcileMode: "none", GenericScopeJSONPath: "", PathParams: []dependentPathParamDef{
			{Param: "agent_id", Field: "id"},
		}},
	}
}
