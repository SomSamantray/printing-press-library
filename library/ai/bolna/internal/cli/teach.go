...
// bolna-pp-cli CLI: `teach` (fire-and-forget, silent on success,
...
func learnDBPath(explicit string) string {
	...
	return defaultDBPath("bolna-pp-cli")
}
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  bolna-pp-cli teach --query "<question>" --resource-type <type> \
...
when learnings exist.`,
		Example: `  bolna-pp-cli recall "<question>" --agent
  bolna-pp-cli recall "<question>" --agent --min-confidence 2`,
...
func newLearningsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  bolna-pp-cli learnings list --agent
  bolna-pp-cli learnings list --query "<substring>"
  bolna-pp-cli learnings list --warnings --agent`,
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
...
Requires at least one of --resource, --action, or --all.`,
		Example: `  bolna-pp-cli learnings forget "<question>" --resource <id>
  bolna-pp-cli learnings forget "<question>" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
...
a whole family.`,
		Example: `  bolna-pp-cli teach-pattern \
...
cannot be taught — they are derived from the canonical input.`,
		Example: `  bolna-pp-cli teach-lookup --kind country --canonical "United States" --value USA`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
			}
