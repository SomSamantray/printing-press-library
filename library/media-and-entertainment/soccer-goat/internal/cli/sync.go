...
Resource scoping:
  ...
  soccer-goat-pp-cli sync

  # Sync specific resources only
  soccer-goat-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  soccer-goat-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  soccer-goat-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  soccer-goat-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  soccer-goat-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(resources) == 0 {
				if humanFriendly {
					fmt.Fprintln(os.Stderr, "soccer-goat-pp-cli sync: no default sync resources in spec; use --resources for explicit sync targets or populate the store via single-fetch commands.")
				} else {
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "clubs":
		return db.UpsertClubs(data)
	case "clubs_players":
		return db.UpsertClubsPlayers(data)
	case "clubs_profile":
		return db.UpsertClubsProfile(data)
	case "competitions":
		return db.UpsertCompetitions(data)
	case "competitions_clubs":
		return db.UpsertCompetitionsClubs(data)
	case "players":
		return db.UpsertPlayers(data)
	case "achievements":
		return db.UpsertAchievements(data)
	case "injuries":
		return db.UpsertInjuries(data)
	case "jersey_numbers":
		return db.UpsertJerseyNumbers(data)
	case "market_value":
		return db.UpsertMarketValue(data)
	case "players_profile":
		return db.UpsertPlayersProfile(data)
	case "stats":
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"injuries": "/players/{player_id}/injuries",
	}
...
func responsePathForResource(resource, path string) []string {
	...
	case "clubs\x00/clubs/search/{club_name}":
		return []string{"results"}
	case "competitions\x00/competitions/search/{competition_name}":
		return []string{"results"}
	case "players\x00/players/search/{player_name}":
		return []string{"results"}
	}
	return nil
}
