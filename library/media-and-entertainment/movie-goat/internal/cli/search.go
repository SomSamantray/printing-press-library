...
In local mode: searches locally synced data only.`,
  ...
  movie-goat-pp-cli search "error timeout"

  # Force local search only
  movie-goat-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  movie-goat-pp-cli search "critical" --type transactions --data-source local
  ...
  movie-goat-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /search/tv
