...
// TestUpsertBatch_PopulatesApisTable verifies that UpsertBatch
// dispatches paginated items into both the generic resources table AND the
// typed apis table. Regression for issue #268: before the fix, paginated
// syncs only filled the generic resources table, so domain commands that
// query the typed table saw zero rows.
func TestUpsertBatch_PopulatesApisTable(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if _, _, err := s.UpsertBatch("apis", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "apis").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	if generic != len(items) {
		t.Fatalf("resources count = %d, want %d", generic, len(items))
	...
	var typed int
	typedQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, "apis")
	if err := db.QueryRow(typedQuery).Scan(&typed); err != nil {
		t.Fatalf("count apis: %v", err)
	}
	if typed != len(items) {
		t.Fatalf("apis count = %d, want %d (typed table not populated by UpsertBatch)", typed, len(items))
	}
}

func TestSearchApisQuotesFTSQuerySyntax(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	for _, query := range []string{
		...
		results, err := s.SearchApis(query, 10)
		if err != nil {
			t.Fatalf("SearchApis(%q): %v", query, err)
		}
		if len(results) == 0 {
			t.Fatalf("SearchApis(%q) returned no results", query)
		}
	}
}
...
func TestSearchMarketplaceQuotesFTSQuerySyntax(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if stored, failed, err := s.UpsertBatch("marketplace", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	} else if failed != 0 || stored != len(items) {
		t.Fatalf("UpsertBatch stored=%d failed=%d, want stored=%d failed=0", stored, failed, len(items))
	}
	...
	for _, query := range []string{
		...
		results, err := s.SearchMarketplace(query, 10)
		if err != nil {
			t.Fatalf("SearchMarketplace(%q): %v", query, err)
		}
		if len(results) == 0 {
			t.Fatalf("SearchMarketplace(%q) returned no results", query)
		}
	}
}
