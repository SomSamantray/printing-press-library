...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/developer-tools/rapidapi/internal/learn"
...
type rootFlags struct {
	...
	// invocation. Mirrors the RAPIDAPI_NO_LEARN env var.
	noLearn                 bool
	selectFields            string
	configPath              string
	homePath                string
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "rapidapi-pp-cli",
		Short: "Manage rapidapi resources via the rapidapi API",
		Long: `Manage rapidapi resources via the rapidapi API.

Add --agent to any command for JSON output + non-interactive mode.
Run 'rapidapi-pp-cli doctor' to verify auth and connectivity.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("rapidapi-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.selectFields, "select", "", "Comma-separated fields to include in output (e.g. --select id,name,weight)")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'rapidapi-pp-cli profile list')")
	...
	rootCmd.PersistentPreRunE = func(cmd *cobra.Command, args []string) error {
		...
		// Auto-refresh the local cache when stale (schema-gated, offline-safe).
	...
	rootCmd.AddCommand(newApisPromotedCmd(flags))
	rootCmd.AddCommand(newCategoriesPromotedCmd(flags))
	rootCmd.AddCommand(newMarketplacePromotedCmd(flags))
	rootCmd.AddCommand(newMetricsPromotedCmd(flags))
	...
	rootCmd.AddCommand(newUsagesCmd(flags))
	rootCmd.AddCommand(newSeoCmd(flags))
	...
	rootCmd.AddCommand(newFollowersCmd(flags))
	rootCmd.AddCommand(newRatingsCmd(flags))
	rootCmd.AddCommand(newAnnouncementsCmd(flags))
	rootCmd.AddCommand(newSpotlightsCmd(flags))
	rootCmd.AddCommand(newGatewaysCmd(flags))
	rootCmd.AddCommand(newCertificatesCmd(flags))
	rootCmd.AddCommand(newTutorialsCmd(flags))
