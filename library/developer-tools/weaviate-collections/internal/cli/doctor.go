...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  weaviate-collections-pp-cli doctor
  weaviate-collections-pp-cli doctor --json
  weaviate-collections-pp-cli doctor --fail-on warn
  weaviate-collections-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if os.Getenv("WEAVIATE_API_KEY") != "" {
				authEnvSet = append(authEnvSet, "WEAVIATE_API_KEY")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "WEAVIATE_API_KEY")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "weaviate-collections-pp-cli", suggestion)
						} else {
				...
				report["api"] = "not configured (set WEAVIATE_COLLECTIONS_BASE_URL, or base_url in the config file, to your Weaviate Cloud cluster URL)"
			}
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	report := map[string]any{}
	dbPath := defaultDBPath("weaviate-collections-pp-cli")
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'weaviate-collections-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'weaviate-collections-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "empty"
		report["hint"] = "Cache is empty; run 'weaviate-collections-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'weaviate-collections-pp-cli sync' to refresh."
	}
	return report
}
