...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/developer-tools/algolia/internal/learn"
	"github.com/mvanhorn/printing-press-library/library/developer-tools/algolia/internal/learn/lookups"
...
Resource scoping:
  ...
  algolia-pp-cli sync --resources clusters,clusters-mapping

  # Full resync (ignore previous checkpoint)
  algolia-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  algolia-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  algolia-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  algolia-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dbPath == "" {
				dbPath = defaultDBPath("algolia-pp-cli")
			}
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "clusters":
		return db.UpsertClusters(data)
	case "dictionaries_batch":
		return db.UpsertDictionariesBatch(data)
	case "search":
		return db.UpsertSearch(data)
	case "indexes":
		return db.UpsertIndexes(data)
	case "indexes_batch":
		return db.UpsertIndexesBatch(data)
	case "browse":
		return db.UpsertBrowse(data)
	case "clear":
		return db.UpsertClear(data)
	case "delete_by_query":
		return db.UpsertDeleteByQuery(data)
	case "facets":
		return db.UpsertFacets(data)
	case "operation":
		return db.UpsertOperation(data)
	case "partial":
		return db.UpsertPartial(data)
	case "query":
		return db.UpsertQuery(data)
	case "rules":
		return db.UpsertRules(data)
	case "settings":
		return db.UpsertSettings(data)
	case "synonyms":
		return db.UpsertSynonyms(data)
	case "indexes_task":
		return db.UpsertIndexesTask(data)
	case "keys":
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{ // #nosec G101 -- endpoint paths, not credentials.
		"clusters":             "/1/clusters",
		"clusters-mapping":     "/1/clusters/mapping",
		"clusters-mapping-top": "/1/clusters/mapping/top",
		"dictionaries":         "/1/dictionaries/*/languages",
		"indexes":              "/1/indexes",
		"keys":                 "/1/keys",
		"logs":                 "/1/logs",
		"security":             "/1/security/sources",
	}
...
var reconcileTypedTables = map[string]string{
	...
	"dictionaries_batch": "dictionaries_batch",
	"facets":             "facets",
	"indexes":            "indexes",
	"indexes_batch":      "indexes_batch",
	"indexes_task":       "indexes_task",
	"keys":               "keys",
	"logs":               "logs",
	"operation":          "operation",
	"partial":            "partial",
...
func responsePathForResource(resource, path string) []string {
	...
	case "clusters\x00/1/clusters/mapping/top":
		return []string{"topUsers"}
	case "clusters\x00/1/clusters":
		return []string{"topUsers"}
	case "clusters\x00/1/clusters/mapping":
		return []string{"userIDs"}
	case "clusters\x00/1/clusters/mapping/search":
		return []string{"hits"}
	case "indexes\x00/1/indexes/*/objects":
		return []string{"results"}
	case "indexes\x00/1/indexes":
		return []string{"items"}
	case "indexes\x00/1/indexes/*/batch":
		return []string{"objectIDs"}
	case "indexes\x00/1/indexes/*/queries":
		return []string{"results"}
	case "keys\x00/1/keys":
		return []string{"keys"}
	case "logs\x00/1/logs":
		return []string{"logs"}
	}
	return nil
}
