...
type rootFlags struct {
	...
	// invocation. Mirrors the QUICKCOMMERCE_NO_LEARN env var.
	noLearn                 bool
	selectFields            string
	configPath              string
	homePath                string
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "quickcommerce-pp-cli",
		Short: `Compare live Indian product prices, stock, and delivery ETAs from the terminal`,
		Long: `Compare live Indian product prices, stock, and delivery ETAs from the terminal
...
Health check: run 'quickcommerce-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("quickcommerce-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.selectFields, "select", "", "Comma-separated fields to include in output (e.g. --select query,platforms,lat)")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'quickcommerce-pp-cli profile list')")
	...
	rootCmd.AddCommand(newDeliveryCmd(flags))
	...
	rootCmd.AddCommand(newComparisonPromotedCmd(flags))
	rootCmd.AddCommand(newItemsPromotedCmd(flags))
	rootCmd.AddCommand(newPlatformsPromotedCmd(flags))
	...
	addNovelCommandIfAbsent(rootCmd, newNovelMirrorCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelPricesCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelRequestsCmd(flags))
