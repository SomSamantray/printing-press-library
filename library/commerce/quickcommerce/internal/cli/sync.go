...
Resource scoping:
  ...
  quickcommerce-pp-cli sync
  # Sync specific resources only
  quickcommerce-pp-cli sync --resources comparison,delivery

  # Full resync (ignore previous checkpoint)
  quickcommerce-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  quickcommerce-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  quickcommerce-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  quickcommerce-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dbPath == "" {
				dbPath = defaultDBPath("quickcommerce-pp-cli")
			}
...
func syncResource(ctx context.Context, c interface {
	...
	for {
		...
		applyQuickCommerceSyncDefaults(resource, params)
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "comparison":
		return db.UpsertComparison(data)
	case "delivery":
		return db.UpsertDelivery(data)
	case "items":
		return db.UpsertItems(data)
	case "platforms":
		return db.UpsertPlatforms(data)
	case "products":
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{ // #nosec G101 -- endpoint paths, not credentials.
		"comparison": "/v1/groupsearch",
		"delivery":   "/v1/groupeta",
		"items":      "/v1/item",
		"platforms":  "/v1/supported-platforms",
		"products":   "/v1/search",
	}
...
func responsePathForResource(resource, path string) []string {
	...
	case "comparison\x00/v1/groupsearch":
		return []string{"data"}
	case "delivery\x00/v1/groupeta":
		return []string{"data"}
	case "delivery\x00/v1/eta":
		return []string{"data"}
	case "items\x00/v1/item":
		return []string{"data.items"}
	case "products\x00/v1/search":
		return []string{"data.products"}
	}
	return nil
}
